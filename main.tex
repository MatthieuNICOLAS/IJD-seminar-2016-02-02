...
\begin{frame}{Presentation of PLM}{Application's architecture}
  \begin{center}
    \begin{tikzpicture}[
      ...
      \node[right=3 of user] (center-webPLMs) {};
      \node[block, above=0.3 of center-webPLMs] (webPLM) { webPLM-central };
      \node[block, below=0.3 of center-webPLMs] (webPLM-local) { webPLM-local };
      \node[draw, densely dotted, fit=(webPLM) (webPLM-local)] (webPLMs) {};

      \node[block, right=of webPLMs] (plm-data) { PLM-data };
      ...
      \path[<->, shorten >=3pt, shorten <=3pt]
        ...
        (webPLMs) edge (plm-data)
        (webPLMs) edge[bend right] node[above right, align=center, xshift=-5, scale=0.6] {retrieve user's \\ preferences} (plm-profiles.west)
        (webPLMs) edge[bend left] node[below right, align=center, yshift=-15, xshift=-25, scale=0.6]  {validate \\ authentification} (oauth-providers.east);
...
\begin{frame}{Assessment of user's code}{Implementation}
  \begin{itemize}
  \item {
