...
\begin{frame}{User's code's assessment}{More about \emph{docker run}}
  \begin{itemize}
  \item {
    Can also manage
    \begin{itemize}
...
\begin{frame}{Result}{Current architecture}
  \begin{center}
    \begin{tikzpicture}[
      ...
      \node[block, right=of webPLMs] (plm-profiles) { PLM-profiles };

      \path[<->, shorten >=3pt, shorten <=3pt]
        (webPLMs.east) edge (plm-profiles.west);
      ...
      \path[<->, shorten >=3pt, shorten <=3pt]
        ...
        (webPLMs.south)[xshift=-10] edge[bend left] (oauth-providers.east);

      %%%%%%%%%%%%%%%%%%
      % Judges
      %%%%%%%%%%%%%%%%%%
      \node[below right =0.8 of webPLMs.south, xshift=10] (mq) {\includegraphics[scale=0.3]{img/message-queue-2.png}};
      \node[block, below right=0.5 of mq] (judges) {Judges};

      \path[<->, shorten >=2pt, shorten <=2pt]
        (webPLMs.south)[xshift=5] edge[bend right] (mq.west)
